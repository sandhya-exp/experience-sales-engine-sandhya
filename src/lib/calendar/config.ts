 ...
 *   SALES_HOURS                     "09:30-17:00" local business hours. Default 09:30-17:00, Mon–Fri.
...
export function schedulingConfig(): SchedulingConfig {
  const hours = (process.env.SALES_HOURS ?? "09:30-17:00").match(/^(\d{1,2}):(\d{2})-(\d{1,2}):(\d{2})$/);
  const toMin = (h: string, m: string) => Number(h) * 60 + Number(m);
  return {
    timeZone: process.env.SALES_TIMEZONE?.trim() || "America/New_York",
    hours: hours ? { start: toMin(hours[1], hours[2]), end: toMin(hours[3], hours[4]) } : { start: 9 * 60 + 30, end: 17 * 60 },
